...
export const ChatRoom: React.FC<TWebSocket> = ({ socket }): JSX.Element => {
  ...
  useEffect(() => {
    ...
    if (chatRoomDetails && chatRoomDetails.id === id) {
      ...
      setIsAdmin(
        (chatRoomDetails.isGroup ? chatRoomDetails.admins?.some((admin) => admin.id === user.id) : false) || false
      );
          ...
          switch (responseMessage?.type) {
              ...
              if (responseMessage?.status === StatusCodes.Forbidden) {
                switch (responseMessage?.payload?.action) {
                  ...
                  case "select-superadmin":
                    ...
                    const potentialSuperAdmins = responseMessage?.payload?.participants as TParticipant[];
                  ...
                  case "auto-superadmin":
                    const newSuperAdmin = (responseMessage?.payload?.participants as TParticipant[])?.filter(
              ...
              if (responseMessage.success === false) {
                ...
                if (responseMessage.status === StatusCodes.Ok && chatRoomDetails.isGroup === true) {
                  const updatedChatRoomDetails = {
                    ...
                    participants: chatRoomDetails.participants.filter((member) => member.id !== user.id),
                  } satisfies TChatRoomAtom;
            ...
            case DELETE_CHAT:
              if (responseMessage.success === false) {
                ...
                if (responseMessage.status === StatusCodes.Ok) {
                  toast.success(responseMessage?.payload?.isGroup ? "Group deleted!" : "All messages deleted!");
                  navigate(NAVIGATION_ROUTES.INBOX, { replace: true });
                  return;
                }
                commonToastErrorMessage({
                ...
                switch (responseMessage.status) {
                  ...
                  case StatusCodes.Conflict:
                    ...
                    setChatRoomDetails((prev) =>
                      prev && prev.isGroup
                        ? {
                            ...prev,
                            participants: prev.participants.filter((member) => member.id !== removeUserId),
                  ...
                  case StatusCodes.Forbidden:
                    if (action === "permission-denied") {
                      toast.error("You do not have the required permissions to remove members from this group");
                      setChatRoomDetails((prev) =>
                        prev && prev.isGroup
                          ? {
                              ...prev,
                ...
                if (responseMessage?.status === StatusCodes.Ok) {
                  ...
                  setChatRoomDetails((prev) =>
                    prev && prev.isGroup
                      ? {
                          ...prev,
                          participants: prev.participants.filter((member) => member.id !== removedMemberId),
            ...
            case NEW_MESSAGE:
              if (responseMessage.status === StatusCodes.Ok) {
                const chatRoomId = (responseMessage?.payload as TNewMesageResponse).messageDetails.chatRoomId;
                if (chatRoomId !== id) {
                  return;
                }
                const newMessage = (responseMessage?.payload as TNewMesageResponse).messageDetails.latestMessage;
      ...
      >
        ...
        <div className="border-input flex w-full items-center justify-between border-b p-4">
          <div className="flex items-center gap-3">
            ...
            />
            <Avatar className="size-12 cursor-pointer select-none" onClick={() => setIsRoomInfoVisible((p) => !p)}>
              <AvatarImage src={chatRoomImage} alt="Group Image" />
              <AvatarFallback>{getAvatarFallback(chatRoomName)}</AvatarFallback>
            </Avatar>
            <h1 className="cursor-pointer select-none font-semibold" onClick={() => setIsRoomInfoVisible((p) => !p)}>
              {chatRoomName}
            </h1>
          </div>
          <CircleAlert
                  ...
                  <div className="flex flex-col items-end gap-1" key={message.id}>
                    <div className="flex max-w-[75%]">
                      ...
                      <Avatar className="size-6 self-end">
                        ...
                        <AvatarFallback>{message?.sentBy?.username?.slice(0, 2)?.toUpperCase()}</AvatarFallback>
                      </Avatar>
                    </div>
                    {index === chatRoomDetails.messages.length - 1 && isSendingMessage ? (
                      <Clock className="text-muted-foreground size-3" />
                    ) : (
                      <span className="text-muted-foreground text-xs">{format(message?.sentAt, "HH:mm")}</span>
                    )}
                  </div>
                ) : (
      ...
      {isRoomInfoVisible && (
        <div className="border-input ml-[1px] flex h-full w-full flex-col border-l xl:w-[550px]">
          <div className="border-input flex items-center gap-3 border-b p-6">
            <ArrowLeft className="size-7 active:brightness-50 xl:hidden" onClick={() => setIsRoomInfoVisible(false)} />
                    ...
                    <div className="flex w-full items-center justify-between" key={member.id}>
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col gap-1">
                          <p className="line-clamp-1 text-ellipsis text-sm font-bold">{member.username}</p>
                          <div className="flex items-center text-xs font-bold text-gray-400">
                            {isUserAdmin && (
                              <span className="after:px-1 after:text-gray-400 after:content-['·']">Admin</span>
                            )}
                      ...
                      {isAdmin && !isUserSuperAdmin && member.id !== user.id && (
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent>
                            ...
                            <DropdownMenuItem
                              onClick={() =>
                                handleAdminStatusChangeConfirmation(member.id, member.username, isUserAdmin)
                              }
                            >
