...
export class InboxManager {
  ...
  async handleNewMessage(userId: string, message: ISendMessage) {
    try {
      ...
      if (!chatRoom) {
        this.res.error(SEND_MESSAGE, "Chat room not found or user not a part of this chat room", STATUS_CODE.FORBIDDEN);
        return;
      }
  ...
  async handleChatRoomCreation(id: string, message: IRoomExistsRequest) {
    ...
    if (existingChatRoom) {
      ...
      this.res.status(STATUS_CODE.CONFLICT).json(ROOM_EXISTS, { existingChatRoom: modifiedChatRoom, isGroup: false });
      return;
    }
  ...
  async handleChangeGroupName(id: string, message: IUpdateChatRoomName) {
    ...
    if (!isMember?.id) {
      this.res.error(CHANGE_GROUP_NAME, "Only members of the group can change the group name", STATUS_CODE.FORBIDDEN);
      return;
    }
  ...
  async addUserToChat(id: string, message: IAddUserToGroupChat) {
    ...
    if (!chatRoom || !chatRoom.id) {
      this.res
        .status(STATUS_CODE.NOT_FOUND)
        .json(REMOVE_FROM_CHAT, { message: "This chat room does not exists", action: "chat-room" }, { success: false });
      return;
    }
    ...
    for (const userId of addUsersId) {
      ...
      if (!user?.id) {
        this.res
          .status(STATUS_CODE.NOT_FOUND)
          .json(ADD_TO_CHAT, { message: "This user does not exists", user, action: "user" }, { success: false });
        return;
      }
  ...
  async removeUserFromChat(id: string, message: IRemoveUserFromGroupChat) {
    ...
    if (!chatRoomId || !removeUserId) {
      this.res
        .status(STATUS_CODE.BAD_REQUEST)
        .json(REMOVE_FROM_CHAT, { message: "Invalid parameters", action: "invalid-params" }, { success: false });
      return;
    }
    ...
    try {
      ...
      if (!isGroup) {
        ...
        if (!isUserMember) {
          this.res
            .status(STATUS_CODE.BAD_REQUEST)
            .json(DELETE_CHAT, { message: "Not a part of this chat", action: "not-member" }, { success: false });
          return;
        }
      ...
      if (!hasPermission) {
        this.res.error(DELETE_CHAT, "You do not have permission to delete this group chat", STATUS_CODE.FORBIDDEN);
        return;
      }
